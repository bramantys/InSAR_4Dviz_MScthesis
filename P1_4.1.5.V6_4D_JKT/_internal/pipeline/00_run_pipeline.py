...
ERROR_PATTERNS = [
    # Only explicit failure/error markers should classify a successful step as erroneous.
    # Avoid broad ``\bERROR\b`` matching because benign diagnostics such as
    # "Parent geometric error: 100.0 m" are normal 3D Tiles terminology.
    r"^\s*\[FAIL\]",
    r"^\s*FAIL(?:ED)?(?:\s|:|$)",
    r"^\s*\[ERROR\]",
    r"^\s*ERROR(?:\s|:|$)",

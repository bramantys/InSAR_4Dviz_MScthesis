// ============================================================
// HORIZONTAL PARTICLES ENGINE — Batch 9.3.6
// ============================================================
...
//   primitive_points = Cesium PointPrimitiveCollection dots, depth-tested, no trails yet
//
// Canvas remains as fallback/comparison for now. Monte Carlo uncertainty
// is not implemented here yet; this file is the engine foundation.
// Batch 9.3.6 reads viewer/runtime defaults from viewer_tuning.json via globals set by the HTML.
...
async function loadHParticleHeightTexture(meta) {
    ...
    hCurrentSigmaByRow = new Float32Array(NUM_RUMS);
    ...
    console.log("[H particles] Dynamic surface height enabled", {
        ...
        note: "Canvas particles project at datum + MODEL displacement_mm * vertical scale_m_per_mm + offset"
    });
}
...
function updateHParticleDisplacementForEpoch(epochIdx) {
    ...
    for (let row = 0; row < NUM_RUMS; row++) {
        ...
        if (hCurrentSigmaByRow) {
            hCurrentSigmaByRow[row] = decodeSigmaMmFromByte(col[k + 2]);
        }
    }
...
function hParticleRendererContractLabel() {
    if (isHParticlePrimitiveMode()) return "scene-space primitive dots, depth-tested, no trails yet";
    return "screen-space canvas trails, no true depth test";
}
...
function updateHorizontalLegendLabels() {
    ...
    if (hLegendLeftText) {
        hLegendLeftText.textContent = `Canvas trails | P75 ${speedTxt} mm/yr`;
    }
    if (hLegendRightText) {
...
function seedHLegendParticle(width, height, randomX = true) {
    return {
        x: randomX ? Math.random() * width : -8 - Math.random() * width * 0.15,
        yBase: 5 + Math.random() * Math.max(6, height - 10),
        y: null,
        prevX: null,
        prevY: null,
        phase: Math.random() * Math.PI * 2.0,
        freq: 0.8 + Math.random() * 1.4,
        ampScale: 0.55 + Math.random() * 0.75,
    };
}
...
function setH4Status() {
    setStatus(`loaded ✓  |  H particles: ${hParticleEngineLabel()} / ${hSamplerLabel()} + Step 17 arrows/ellipses`);
}
...
function drawHorizontalParticles(timestamp) {
    ...
    for (const p of hParticles) {
        ...
        hParticleAdvanceLonLat(p, field, dt);
...
function setHorizontalUncertaintyVisible(visible) {
    ...
    setLayerToggleButtonState(hUncertaintyToggleBtn, visible, "H uncertainty");
    resetHParticleScreenHistory();
    clearHParticlesCanvas();
    syncLegendActiveStates();
}

...
DISPLACEMENT_PARQUET = OUTPUT_DATA / "parcel_displacement_long.parquet"
...
def main():
    ...
    # Read canonical displacement product, only ID set.
    print(f"\nReading canonical displacement product:\n  {DISPLACEMENT_PARQUET}")
    disp_ids = pd.read_parquet(DISPLACEMENT_PARQUET, columns=["pnt_id"])
    moving_ids = set(pd.to_numeric(disp_ids["pnt_id"], errors="raise").astype("int64").unique().tolist())

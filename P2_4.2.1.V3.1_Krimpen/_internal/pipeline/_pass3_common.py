...
def load_legacy_module(pipeline_dir: Path, filename: str, module_name: str):
    path = require(pipeline_dir / filename, f"legacy builder {filename}")
    spec = importlib.util.spec_from_file_location(module_name, path)
    if spec is None or spec.loader is None:
        raise Pass3Error(f"Could not import legacy builder: {path}")
...
def stage_root(project_root: Path) -> Path:
    return project_root / "_internal" / "data_pipeline" / "_stage"


def semantic_root(project_root: Path) -> Path:
    return project_root / "_internal" / "data_pipeline"

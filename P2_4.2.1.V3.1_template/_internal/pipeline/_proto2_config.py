...
def input_path(
    ...
    mapping = {
        ...
        ("displacement", "path"): "displacement_csv",
...
def runtime_root(project_root: Path, config: dict[str, Any]) -> Path:
    value = get_nested(config, "paths", "runtime_root", default="_internal/data_pipeline")
    return rel_path(project_root, value)


def output_data_dir(project_root: Path, config: dict[str, Any]) -> Path:
    value = get_nested(config, "paths", "work_data", default="_internal/data_pipeline/_work/data")
    return rel_path(project_root, value)


def output_cesium_dir(project_root: Path, config: dict[str, Any]) -> Path:
    value = get_nested(config, "paths", "work_cesium", default="_internal/data_pipeline/_work/cesium")
    return rel_path(project_root, value)


def stage_records_dir(project_root: Path, config: dict[str, Any]) -> Path:
    value = get_nested(config, "paths", "stage_records", default="_internal/data_pipeline/_work/stage_records")
    return rel_path(project_root, value)


def viewer_tuning_path(project_root: Path, config: dict[str, Any]) -> Path:
    value = get_nested(config, "paths", "viewer_tuning", default="_internal/data_pipeline/style/viewer_tuning.json")
    return rel_path(project_root, value)
...
def should_require_model_parameters(config: dict[str, Any]) -> bool:
    # All declared user inputs are part of the bonestock input contract, even
    # though displacement is currently read from the precomputed CSV.
...
def active_deformation_source(config: dict[str, Any]) -> str:
    return str(get_nested(config, "pipeline_source", "deformation_source", default="displacement_csv"))


def pyspams_automated(config: dict[str, Any]) -> bool:
    return bool(get_nested(config, "pipeline_source", "pyspams_automated", default=False))

// ============================================================
// HORIZONTAL PARTICLES ENGINE — V4.1 canvas uncertainty comparison
// ============================================================
...
function hParticleRendererContractLabel() {
    ...
    if (hParticleUncertaintyMode === "montecarlo") return "screen-space canvas trails + Monte Carlo path realization";
...
function updateHorizontalLegendLabels() {
    ...
    if (hLegendRightText) {
        if (hParticleUncertaintyMode === "montecarlo") {
            hLegendRightText.textContent = `Velocity realization | scale ${hUncertaintyStrength.toFixed(2)}`;
...
function resetHParticleMonteCarloRealization(p) {
    if (!p) return;
    p.mcZ1 = randomNormal01();
    p.mcZ2 = randomNormal01();
}

function covarianceRealizationOffset(field, z1, z2) {
    const ve = Math.max(0.0, Number(field?.var_east || 0.0));
    const vn = Math.max(0.0, Number(field?.var_north || 0.0));
    const c = Number(field?.covar_en || 0.0);

    // Symmetric 2x2 eigen decomposition, with PSD clamping.
    const mid = 0.5 * (ve + vn);
    const diff = 0.5 * (ve - vn);
    const root = Math.sqrt(Math.max(0.0, diff * diff + c * c));
    const lambda1 = Math.max(0.0, mid + root);
    const lambda2 = Math.max(0.0, mid - root);
...
function hParticleMotionField(field, particle) {
    ...
    const offset = covarianceRealizationOffset(field, particle.mcZ1, particle.mcZ2);
    const scale = Math.max(0.0, Number(hUncertaintyStrength || 0.0));
    const east = Number(field.east || 0.0) + offset.east * scale;
    const north = Number(field.north || 0.0) + offset.north * scale;
